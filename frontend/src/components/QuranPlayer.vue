...
const emit = defineEmits<{ 'playback-started': []; 'repetition-complete': [verseKey: string]; 'range-complete': []; 'playback-error': [message: string]; 'verse-highlight': [verseKey: string | null]; 'word-progress': [progress: { verseKey: string; fraction?: number; wordIndex?: number } | null] }>()
const audio = ref<HTMLAudioElement | null>(null)
const verseIndex = ref(0)
const repeatIndex = ref(1)
const playing = ref(false)
const segmentMap = ref<Record<string, { time_from: number; time_to: number; segments?: [number, number, number][] }>>({})
const surahHighlightVerseKey = ref<string | null>(null)
...
const highlightedVerse = computed(() => {
  ...
  if (props.recitation?.capability === 'surah') return surahHighlightVerseKey.value
  return current.value ? current.value.verse_key : null
})
...
// ponytail: standalone ayah recordings have no per-word timing source, so this assumes uniform word duration; upgrade if a word-timing endpoint for single-ayah audio becomes available
...
const wordIndexFromSegments = (segments: [number, number, number][] | undefined, elapsedMs: number) => {
  if (!Array.isArray(segments) || !segments.length || !Number.isFinite(elapsedMs)) return -1
  const index = segments.findIndex(([, , to]) => elapsedMs < to)
  return Math.max(0, index === -1 ? segments.length - 1 : index)
}
const emitSegmentedWordProgress = () => {
  ...
  const wordIndex = segment ? wordIndexFromSegments(segment.segments, currentTime) : -1
  if (!segment || !Number.isFinite(currentTime) || wordIndex === -1) { emit('word-progress', null); return }
  emit('word-progress', { verseKey: current.value.verse_key, wordIndex })
}
const emitSurahWordProgress = () => {
  if (!playing.value || !audio.value) { surahHighlightVerseKey.value = null; emit('word-progress', null); return }
  const currentTime = audio.value.currentTime * 1000
  const entry = Object.entries(segmentMap.value).find(([, segment]) => currentTime >= segment.time_from && currentTime < segment.time_to)
  surahHighlightVerseKey.value = entry ? entry[0] : null
  const wordIndex = entry ? wordIndexFromSegments(entry[1].segments, currentTime) : -1
  emit('word-progress', wordIndex === -1 ? null : { verseKey: entry![0], wordIndex })
}
const onEnded = async () => {
...
const onTimeUpdate = async () => {
  ...
  if (props.recitation?.capability === 'surah') emitSurahWordProgress()
...
const loadSegments = async () => {
  const capability = props.recitation?.capability
  if ((capability !== 'segmented_surah' && capability !== 'surah') || !props.verses.length) { segmentMap.value = {}; return }
  try { const { data } = await api.get(`/quran/recitations/${props.recitation.id}/segments`, { params: { surah_id: props.verses[0].surah_id, start_ayah: props.verses[0].ayah_number, end_ayah: props.verses.at(-1).ayah_number } }); segmentMap.value = data.segments || {} } catch { segmentMap.value = {}; if (capability === 'segmented_surah') failed() }
}
