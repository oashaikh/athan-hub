...
class QuranCacheService:
    ...
    def cache_summary(self, db: Session) -> dict:
        rows = db.query(models.QuranAudioCache).all()
        return {
...
def audio_source(recitation: dict, verse_key: str | None, surah_id: int) -> tuple[str, str]:
    ...
    request = urllib.request.Request(
        recitation["detail_url"], headers={"User-Agent": "Athan-Hub-Quran-Catalogue/1.0"}
    )
    with urllib.request.urlopen(request, timeout=120) as response:
        detail = json_module.load(response)["recitation"]
    if recitation["source_kind"] == "ayah":
        if not verse_key:
            raise CacheSourceError("Ayah recitations require a verse key")
        row = detail["audio_files"].get(verse_key)
        if not row:
            raise CacheSourceError("Selected recitation has no audio for this verse")
        return verse_key, row["audio_url"]
    row = detail["audio_files"].get(str(surah_id))
    if not row:
        raise CacheSourceError("Selected recitation has no audio for this surah")
    return str(surah_id), row["audio_url"]
